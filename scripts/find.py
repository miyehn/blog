...
s = get_input('gimme some text in the post you\'re trying to find:\n> ')
...
while not (len(files)==1 and len(files[0])>0):
    ...
    s = get_input('gimme some other text:\n> ')
    files = os.popen('grep -l "' + s + '" ' + content_dir + '/drafts/*.md').read().strip().split('\n')
...
while not rec_action:
    a = get_input('what do you want to do with it?\nreply [e]dit, [r]ename, [d]elete, or [n]othing:\n> ').lower()
    if a=='e' or a=='edit':
        if pc=='1':
            os.system('start '+f)
        else:
        ...
        new_name = get_input('sure. gimme it\'s new name (not the whole path) (without extension):\n> ')
        while True:
            new_path = content_dir+'/drafts/'+new_name+'.md'
            if os.path.isfile(new_path):
                new_name = get_input('this name\'s taken. gimme another one:\n> ')
            else:
